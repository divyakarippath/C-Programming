...
//
//
//  Created by Atul Prakash on 10/25/15.
//
//
/************************************************************************************************************/
...
{
    int sock_descriptor, new_client_descriptor, server_port, max_clients, max_descriptors, i;
    char *server_ip;
        ...
        for(i=0; i<= max_descriptors; i++)
        {
            ...
            if(FD_ISSET(i, &client_descriptors))
            {
                ...
                else
                {
                    ...
                    long numofbytes = read(i,(struct sbcpmessage *) &incoming_message,sizeof(incoming_message));
                    if(numofbytes > 0)
                    {
...
void process_incoming_message(struct sbcpmessage incoming_message, struct clientdetails client_list[], int client_descriptor, int max_clients)
{
    ...
    strftime(timed, 20, "%H:%M:%S", tm_info);  // Time format : MM/DD/YYYY HH:MM:SS Weekday
